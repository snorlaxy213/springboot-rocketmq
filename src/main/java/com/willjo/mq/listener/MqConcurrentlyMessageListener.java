...
@RocketMqListener(topic = MqConstant.Top.CONSOLE_USER_ORDER_TOPIC, consumerGroup = ConsumeGroup.CONSOLE_USER_GROUP)
public class MqConcurrentlyMessageListener implements MessageListener {
    ...
    public MqAction consume(MessageExt message, ConsumeConcurrentlyContext context) {
        ...
        try {
            ...
            LOGGER.info("MsgId:{},MQ消费,Topic:{},Tag:{}，Body:{}",
                    message.getMsgId(), message.getTopic(), message.getTags(), msg);
        } catch (Exception e) {
            LOGGER.error("MsgId:{},应用MQ消费失败,Topic:{},Tag:{}，Body:{},异常信息:{}",
                    message.getMsgId(), message.getTopic(), message.getTags(), msg, e.toString());
            throw e;
        }
